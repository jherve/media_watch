defmodule MediaWatch.Catalog.SourceWorker do
  ...
  def handle_continue(:parsing, state = %{snapshot: snap}) do
    case ParsingServer.parse(snap, state.module) do
      {:ok, parsed} ->
        {:noreply, state |> Map.put(:parsed_snapshot, parsed), {:continue, :slicing}}
  ...
  def handle_continue(:entities_recognition, state = %{slices: [], slices_recognized: slices}) do
